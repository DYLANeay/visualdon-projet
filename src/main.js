...
// Reload landing should always start at the top: with manual restoration the
// browser won't restore a stale scrollY against a page whose total height is
// still growing as lazy modules render.
...
const europeBootPromise = bootEurope();
...
// Boot Switzerland eagerly: its scroll steps are generated by JS, so until
// bootSwitzerland runs the section is much shorter than its final size.
// A nav click ("Villes/campagnes") from the top would compute a target Y
// against the pre-boot layout, then bootSwitzerland would fire mid-scroll
// and grow the document, leaving the user at the start of the Swiss slider.
const switzerlandBootPromise = bootSwitzerland();

// ── Chapter 3 charts ────────────────────────────────────────────────────────
// Init eagerly so the horizontal-scroll wrapper can measure the real section
// heights up-front. Lazy-loading these via IntersectionObserver let the wrapper
// settle on a too-short height; a fast F5 scroll then crossed it before charts
// rendered, and the user landed at the end of the page.

const cityRuralEl = document.querySelector('#city-rural-chart');
if (cityRuralEl) initCityRuralChart(cityRuralEl);

const languageRegionsEl = document.querySelector('#language-regions-chart');
if (languageRegionsEl) initLanguageRegionsChart(languageRegionsEl);
...
for (const link of document.querySelectorAll('.site-nav-links a[href^="#"]')) {
  link.addEventListener('click', async (e) => {
    ...
    if (!hash) return;
    e.preventDefault();
    // Wait for both sections to settle to their final height. Their scroll
    // steps are generated inside bootEurope / bootSwitzerland; computing a
    // target Y before those resolve gives a stale value and the user lands
    // mid-section. lock + force keep wheel/trackpad input from cancelling
    // the long scroll animation.
    await Promise.all([europeBootPromise, switzerlandBootPromise]);
    const targetY = horizontalApi?.scrollToSection?.(hash);
    if (typeof targetY === 'number') {
      lenis.scrollTo(targetY, { duration: 1.2, lock: true, force: true });
      return;
    }
    const el = document.getElementById(hash);
    if (el) lenis.scrollTo(el, { duration: 1.0, lock: true, force: true });
  });
}
